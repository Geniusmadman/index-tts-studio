...
如果平台链接需要登录或网络访问不稳定，可以展开「下载设置」：

- `启用下载代理`：只影响网络素材解析，不影响大模型配置。
- `Cookie 文件路径`：可填写从浏览器导出的 Netscape 格式 cookies.txt。

处理失败时，页面会显示 `yt-dlp` 返回的错误摘要，用于判断是登录、权限、风控、链接格式还是网络问题。
